...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,mask = None):
        return self.resblocks(x,mask = mask)
...
#         self.ln_final = LayerNorm(width)
    ...
    def forward(self, x: torch.Tensor,clip_image_atts = None,clip_prompt = None):
        ...
        if clip_image_atts is not None and clip_prompt is not None:
            x = torch.cat([x, clip_prompt.to(x.dtype) + torch.zeros_like(clip_prompt,dtype=x.dtype, device=x.device)], dim=1)

            x = self.ln_pre(x)

            x = x.permute(1, 0, 2)  # NLD -> LND
            x = self.transformer(x,mask = clip_image_atts)
